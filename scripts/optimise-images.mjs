...
const RULES = [
  { match: /\/(home-gallery|Exhibition|volunteers)\//i, maxW: 1920, maxH: 1920, q: 82 },
  { match: /\/speakers\//i,                              maxW: 900,  maxH: 900,  q: 85 },
  { match: /\/(team|endorsements)\//i,                   maxW: 800,  maxH: 800,  q: 85 },
  { match: /\/(Logo|media-coverage|Speaker Highlights)\//i, maxW: 500, maxH: 500, q: 88 },
...
async function processImage(file) {
  ...
  try {
    ...
    let buf;
    if (ext === '.png') {
      buf = await pipeline.png({ quality: rule.q, compressionLevel: 9, adaptiveFiltering: true }).toBuffer();
    } else if (ext === '.gif') {
...
async function main() {
  ...
  for (const file of images) {
    ...
    rows.push({ rel, before: fmt(result.before), after: fmt(result.after), saved: fmt(saved), pct });
    console.log(`  ✓ ${rel.padEnd(55)} ${fmt(result.before).padStart(8)} → ${fmt(result.after).padStart(8)}  (${pct}% saved)`);
  }
