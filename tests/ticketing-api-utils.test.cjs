...
test("serves demo ticket events in development when ticketing schema is missing", async () => {
  const { shouldServeDemoTicketEvents } = await import("../src/lib/ticketing-api-utils.js");

  assert.equal(
    shouldServeDemoTicketEvents({
      message: "Could not find the table 'public.ticket_events' in the schema cache",
      nodeEnv: "development",
    }),
    true,
  );
...
test("serves demo ticket events when Supabase admin credentials are missing", async () => {
  ...
  assert.equal(
    shouldServeDemoTicketEvents({
      message: "Missing SUPABASE_SERVICE_ROLE_KEY for server-side Supabase admin client.",
      nodeEnv: "production",
    }),
    true,
  );
