...
function buildSessionDescription(session) {
  const speakerNames = (session.speakersDetailed || [])
  ...
  return (
    ...
    (speakerNames.length > 0
      ? `${session.title} at the Trust and Safety India Festival programme, featuring ${speakerNames.join(', ')}.`
      : `${session.title} at the Trust and Safety India Festival programme.`)
  );
}
  ...
  return (
    <>
      ...
      <main className="bg-[#fdf6ef] pb-16 text-stone-900 dark:bg-stone-950 dark:text-white">
        <BrandedPageHero className="py-14 md:py-20">
          <div className="relative z-10 mx-auto w-full max-w-5xl px-4 text-center md:px-6">
            <p className="mb-3 text-xs font-semibold uppercase tracking-[0.16em] text-white/75">
              {ALL_DAY_LABELS[session.day] || 'TASI Programme'}
            </p>
        ...
        <section
          className={`mx-auto grid w-full max-w-6xl items-stretch gap-8 px-4 py-12 md:px-6 ${
            speakers.length > 0 ? 'md:grid-cols-[1fr_320px]' : ''
          }`}
        >
