...
import { createNotification, createRegistration, markNotificationDelivery } from "@/lib/registration-db";
import { deliverRegistrationEmail } from "@/lib/registration-email";
import { PROFILE_BUCKET, buildRegistrationCode, buildStoragePath, normalizeRegistrationPayload } from "@/lib/registration-utils";
...
export async function POST(request) {
  ...
  try {
    ...
    if (!isValidEmail(registration.email)) {
      return Response.json({ error: "Valid email is required." }, { status: 400, headers: protection.headers });
    }

    if (!(profilePhoto instanceof File) || !profilePhoto.size) {
      return Response.json({ error: "Profile photo is required." }, { status: 400, headers: protection.headers });
    }
    ...
    if (!dimensions.width || !dimensions.height || dimensions.width < MIN_IMAGE_SIZE || dimensions.height < MIN_IMAGE_SIZE) {
      return Response.json(
    ...
    const { error: uploadError } = await supabase.storage.from(PROFILE_BUCKET).upload(storagePath, buffer, {
      contentType: profilePhoto.type,
      upsert: false,
    });

    if (uploadError) {
      return Response.json({ error: uploadError.message }, { status: 500, headers: protection.headers });
    }
