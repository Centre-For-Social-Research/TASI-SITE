...
export async function GET(request) {
  ...
  const [
    ...
    supabase
      ...
      .select(
        'id, status, template_type, total_items, sent_items, failed_items, created_by_email, completed_at, created_at'
      )
      .order('created_at', { ascending: false })
      .limit(20),
  ...
  if (
    ...
    !statusHistoryResult.value.error
  ) {
  ...
  if (
    emailOverridesResult.status === 'fulfilled' &&
    !emailOverridesResult.value.error
  ) {
