...
export async function POST(request) {
  try {
    ...
    if (!providerMessageId) {
      return Response.json({ error: "Missing provider message id." }, { status: 400 });
    }

    await updateNotificationFromWebhook({
      providerMessageId,
      deliveryStatus: mapResendEventType(payload.type),
      failureReason: payload.type === "email.bounced" ? payload?.data?.bounce?.reason || "Email bounced." : null,
