...
const enquiryHref =
  'mailto:info1@csrindia.org?subject=Exhibition%20Enquiry%20%E2%80%94%20TASI%202026';
    ...
    <>
      ...
      <main className="bg-white text-stone-900 dark:bg-stone-950 dark:text-stone-100">
        ...
        <section className="relative bg-gradient-to-br from-[#5c0f4f] via-[#360454] to-[#15002b] px-6 py-section-sm md:px-6 md:py-section-md lg:px-16 lg:py-section-lg">
          <div className="mx-auto grid w-full max-w-[1300px] gap-12 lg:grid-cols-[1fr_1fr] lg:gap-20 lg:items-center">
            ...
            <div className="text-white">
              ...
              <h2 className="text-4xl font-extrabold leading-[1.05] tracking-tight text-white md:text-5xl lg:text-[3.2rem]">
                A premium space for organisations shaping <span className="text-rc-secondary dark:text-white">digital trust.</span>
              </h2>
              <div className="mt-8 space-y-6 text-base leading-[1.65] text-white/90 md:text-[17px]">
                ...
                <div className="flex flex-col gap-4 sm:flex-row mt-8">
                  <a href={enquiryHref} className="inline-flex items-center justify-center rounded-3xl bg-white px-8 py-2.5 text-[16px] font-bold text-[#140f26] transition-transform hover:scale-[1.02] sm:w-auto">
                    Send an Enquiry
                  </a>
                  <a href="#participation-modes" className="inline-flex items-center justify-center rounded-3xl border border-white/20 bg-white/10 px-8 py-2.5 text-[16px] font-bold text-white transition-transform hover:scale-[1.02] sm:w-auto">
                    Explore Modes
                  </a>
                </div>
              </div>
            ...
            <div className="grid gap-5">
               {proofPoints.map((point) => (
                <article key={point.title} className="rounded-[10px] border border-white/20 bg-[#1b0d36] p-6 shadow-2xl">
                  <p className="mb-3 text-[11px] font-black uppercase tracking-[0.18em] text-white/55">Value Proposition</p>
                  <h3 className="text-xl font-bold tracking-tight text-white">{point.title}</h3>
        ...
        <section id="participation-modes" className="bg-[#f7f4ef] py-section-md dark:bg-gray-900/30 md:py-section-lg border-t border-gray-100 dark:border-gray-800">
          <div className="container mx-auto max-w-[1300px] px-4 md:px-6">
            ...
            <div className="grid gap-6 md:grid-cols-3">
               {participationModes.map((mode, i) => (
                 <article key={i} className="flex flex-col rounded-[10px] border border-stone-200 bg-white shadow-lg shadow-stone-200/30 overflow-hidden dark:border-stone-800 dark:bg-gray-900/50">
                    <div className="w-full aspect-[3/2] overflow-hidden border-b border-stone-200 dark:border-stone-800">
                      <Image src={mode.image} alt={mode.title} width={600} height={400} className="w-full h-full object-cover transition-transform duration-500 hover:scale-[1.03]" />
                    </div>
                    <div className="flex flex-col flex-grow p-7">
                      <p className="text-[11px] font-black uppercase tracking-[0.16em] text-stone-500 dark:text-stone-400">Mode 0{i+1}</p>
                      <h3 className="mt-3 text-2xl font-bold tracking-tight text-stone-900 dark:text-white">{mode.title}</h3>
                      <p className="mt-4 text-sm leading-relaxed text-stone-600 dark:text-stone-300 flex-grow">{mode.body}</p>
                      <div className="mt-6 pt-4 border-t border-stone-100 dark:border-gray-800">
                        <p className="text-[12px] font-semibold text-rc-accent dark:text-rc-secondary">Recommended Fit</p>
                        <p className="mt-1 text-sm font-medium leading-snug text-stone-700 dark:text-stone-400">{mode.fit}</p>
                      </div>
                    </div>
                 </article>
               ))}
        ...
        <section className="bg-white py-section-md dark:bg-[#121212] md:py-section-lg border-t border-gray-100 dark:border-gray-800">
          <div className="container mx-auto max-w-[1300px] px-4 md:px-6">
            ...
            <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
              {extendedBranding.map((item, i) => (
                <article
                  key={i}
                  className={`group relative flex flex-col rounded-[10px] border border-stone-200 bg-gray-50 shadow-sm transition-shadow hover:shadow-md dark:border-stone-800 dark:bg-gray-900/50 overflow-hidden ${item.cardClassName ?? ''}`}
                >
                  <div className="relative aspect-[3/2] w-full overflow-hidden border-b border-stone-200 dark:border-stone-800 bg-[#f7f4ef] dark:bg-gray-800/80">
                    <Image
                      src={item.image}
                      alt={item.alt}
                      fill
                  ...
                  <div className="flex flex-col flex-grow p-6">
                    <h3 className="mb-3 text-lg font-bold leading-tight text-stone-900 dark:text-white">{item.title}</h3>
                    <p className="text-sm leading-relaxed text-stone-600 dark:text-stone-400">{item.body}</p>
                  </div>
                </article>
              ))}
            </div>
        ...
        <section className="relative overflow-hidden bg-gradient-to-br from-[#350265] to-[#5c0f4f] py-section-md text-white md:py-section-lg px-4 md:px-6">
           <div className="container mx-auto max-w-[1300px]">
              <div className="grid gap-12 lg:grid-cols-[1.1fr_0.9fr]">
                 <div>
                    <h2 className="text-3xl font-black tracking-tight text-white md:text-4xl lg:text-[2.8rem] leading-[1.1]">
                      Visibility feels more valuable when the room is already relevant.
                    </h2>
                    <p className="mt-6 text-lg leading-relaxed text-white/85 max-w-xl">
                      TASI is not built around passive traffic. It is built around organisations and individuals who already have a stake in trust, safety, policy, and implementation.
                    </p>
                 </div>
                 <div className="grid gap-4 sm:grid-cols-2">
                    {[
                      ...
                      <article key={idx} className="rounded-[10px] bg-white/10 p-6 backdrop-blur-sm border border-white/20">
