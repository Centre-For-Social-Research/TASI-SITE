          ...
          <div className="grid gap-5 sm:grid-cols-3">
            {pathways.map((p) => {
              ...
              return (
                ...
                >
                  <div
                    className={`absolute top-0 left-0 h-1 w-full ${p.accent}`}
                  />
                  <div
                    className={`mb-4 flex h-12 w-12 items-center justify-center rounded-[10px] ${p.iconBg}`}
                  >
                    <Icon className={`h-6 w-6 ${p.iconText}`} />
                  </div>
                  <span
                    className={`mb-3 inline-block rounded-full px-2.5 py-0.5 text-xs font-medium ${p.tagBg} ${p.tagText}`}
                  >
                    {p.tag}
                  </span>
