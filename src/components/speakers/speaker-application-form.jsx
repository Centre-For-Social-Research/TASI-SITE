  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      setStatus({
        type: 'success',
        message:
          'Application received. The TASI team will review it and reach out if there is a fit.',
      });
    } catch (error) {
      setStatus({
        type: 'error',
    ...
    <div className="flex h-full flex-col rounded-[10px] border border-stone-200/80 bg-white shadow-[0_24px_80px_rgba(15,23,42,0.08)] dark:border-slate-800 dark:bg-slate-900 dark:shadow-[0_24px_80px_rgba(0,0,0,0.35)]">
      <div className="border-b border-stone-200/80 px-6 py-6 dark:border-slate-800 sm:px-8">
        ...
        <p className="mt-3 max-w-2xl text-sm leading-relaxed text-stone-600 dark:text-slate-300">
          Share your perspective, the conversation you want to shape, and the
          audience need it responds to.
        </p>
      </div>

      <form
        onSubmit={handleSubmit}
        className="flex h-full flex-col px-6 py-6 sm:px-8 sm:py-7"
      >
        ...
        <div className="mt-auto flex flex-col gap-4 border-t border-stone-200/80 pt-5 dark:border-slate-800 lg:flex-row lg:items-center lg:justify-between">
          ...
          <p className="max-w-sm text-sm leading-relaxed text-stone-500 dark:text-slate-400">
            ...
            <a
              href="mailto:india@trustandsafetyfestival.com?subject=TASI%202026%20Speaker%20Application"
