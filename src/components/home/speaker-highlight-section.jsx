...
export default function SpeakerHighlightSection() {
  ...
  const mappedHighlights = highlights.map((highlight) => {
    ...
    const speakerData = speakers.find(
      (s) =>
        ...
        (highlight.searchName && s.name === highlight.searchName)
    );
    return {
      name: highlight.name,
      designation: speakerData
  ...
  return (
    <section
      id="speakers"
      className="pt-1 pb-section-md md:pt-2 md:pb-section-lg bg-white dark:bg-[#121212]"
    >
      <div className="container mx-auto max-w-[1300px] px-4 md:px-6 flex flex-col items-center">
        ...
        <p className="mx-auto mb-10 max-w-3xl text-center text-stone-700 dark:text-stone-300 text-body-md font-normal leading-relaxed">
          Hear directly from trust and safety leaders across policy, platforms,
          civil society, and international partnerships.
        </p>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2 md:gap-4 w-full">
          {mappedHighlights.map((speaker, index) => (
            <div
              key={index}
              className="relative group aspect-[3/4] overflow-hidden bg-gray-200"
            >
              <Image
                src={speaker.image}
                alt={speaker.name}
