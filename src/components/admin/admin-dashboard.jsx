...
function BigNumber({
  ...
  return (
    <div
      style={{
        padding: '22px',
        borderRight: last ? 'none' : '1px solid var(--adm-line)',
        display: 'flex',
    ...
    >
      ...
      <div
        style={{
          ...
          background:
            'linear-gradient(90deg, var(--adm-accent-soft), transparent 40%)',
        }}
      >
        <span style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <span
            ...
            style={{ color: 'var(--adm-accent)', letterSpacing: '0.2em' }}
          >
            LIVE · OPERATIONS PULSE
          </span>
          <span
            className="adm-mono"
            style={{ fontSize: 11, color: 'var(--adm-ink-3)' }}
          >
          ...
          {[
            {
              type: 'speaker',
              name: 'Abhishek Singh',
              role: 'Additional Secretary, MeitY, Govt. of India',
            },
            {
              type: 'theme',
              label: 'Theme 01',
              desc: 'Responsible AI governance and regulation',
            },
            {
              type: 'speaker',
              name: 'Akash Pugalia',
              role: 'Global Executive, TP',
            },
            {
              type: 'partner',
              name: 'Booking.com',
              desc: 'Corporate Partner · Netherlands',
            },
            {
              type: 'speaker',
              name: 'Amelia Wierda',
              role: 'Legal Counsel for Human Rights, Booking.com',
            },
            {
              type: 'theme',
              label: 'Theme 02',
              desc: 'Platform accountability and safety-by-design',
            },
            {
              type: 'speaker',
              name: 'Abby Roberts',
            ...
            },
            {
              type: 'partner',
              name: 'Meta',
              desc: 'Corporate Partner · Technology Platform',
            },
            {
              type: 'speaker',
              name: 'Andras Molnar',
              role: 'Senior Digital Policy Manager, TUM Think Tank',
            },
            {
              type: 'theme',
              label: 'Theme 03',
              desc: 'Online fraud, scams, and financial exploitation',
            },
            {
              type: 'speaker',
              name: 'Anne Collier',
              role: 'Founder & Executive Director, Net Safety Collaborative',
            },
            {
              type: 'partner',
              name: 'Teleperformance',
              desc: 'Corporate Partner · Digital Business Services',
            },
            {
              type: 'speaker',
              name: 'Akansha Kasera',
              role: 'Senior Market Engagement Manager, GSMA',
            },
            {
              type: 'theme',
              label: 'Theme 04',
              desc: 'Gender-based violence and digital safety',
            },
            {
              type: 'speaker',
              name: 'Anshul Tewari',
              role: 'Founder & CEO, Youth Ki Awaaz',
            },
            {
              type: 'partner',
              name: 'Snapchat',
              desc: 'Corporate Partner · United States',
            },
          ]
            .concat([
              {
            ...
            .map((item, i) => (
              <span
                key={i}
                style={{ marginRight: 40, color: 'var(--adm-ink-2)' }}
              >
...
function ActionFeed({ summary, jobs }) {
  const items = useMemo(() => {
    ...
    const failedJobs = jobs.filter(
      (j) => j?.status === 'needs_retry' || j?.failed_items > 0
    );
    ...
    if (out.length === 0) {
      out.push({
        ...
        detail: 'No pending decisions or failed deliveries.',
        time: 'now',
      });
    }
    ...
    >
      ...
      >
        <div>
          <div
            className="adm-eyebrow"
            style={{ color: 'var(--adm-accent)', marginBottom: 4 }}
          >
            NEEDS YOU NOW
          </div>
          <div
        ...
        {items.map((item, i) => (
          <div
            ...
            style={{
              ...
              padding: '14px 18px',
              borderBottom:
                i === items.length - 1 ? 'none' : '1px solid var(--adm-line)',
...
function Funnel({ summary }) {
  ...
  const conversion =
    total > 0 ? (((summary.checkedIn || 0) / total) * 100).toFixed(1) : '0.0';
  ...
  return (
    ...
    >
      ...
      >
        <div>
          <div className="adm-eyebrow" style={{ marginBottom: 4 }}>
            FUNNEL
          </div>
          <div
            style={{ fontSize: 15, fontWeight: 600, color: 'var(--adm-ink)' }}
        ...
        {stages.map((s) => (
          <div key={s.label}>
            ...
            >
              ...
              <span
                className="adm-mono"
                style={{ fontSize: 12, color: 'var(--adm-ink)' }}
              >
                {s.value}
              </span>
            </div>
...
function StatusGrid({ summary }) {
  ...
  return (
    <div
      className="adm-dash-status-grid"
      style={{
        display: 'grid',
...
function JobsPanel({ jobs }) {
  ...
  const statusColor = {
    ...
    running: 'var(--adm-accent)',
    queued: 'var(--adm-info)',
    needs_retry: 'var(--adm-bad)',
  };
  const statusLabel = {
    done: 'DONE',
    running: 'RUNNING',
...
export default function AdminDashboard() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 22 }}>
      ...
      <div
        className="adm-dash-main-grid"
        style={{ display: 'grid', gridTemplateColumns: '1.6fr 1fr', gap: 22 }}
      >
