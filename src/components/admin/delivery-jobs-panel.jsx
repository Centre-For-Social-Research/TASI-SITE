              ...
              <tbody>
                ...
                {!jobsState.loading &&
                  jobsState.jobs.map((job) => (
                    <tr
                      key={job.id}
                      className={`cursor-pointer border-b border-slate-100 transition hover:bg-slate-50 dark:border-slate-800 dark:hover:bg-slate-800/50 ${jobsState.selectedJobId === job.id ? 'bg-amber-50/40 dark:bg-amber-950/20' : ''}`}
                      onClick={() =>
                        setJobsState((current) => ({
                          ...current,
