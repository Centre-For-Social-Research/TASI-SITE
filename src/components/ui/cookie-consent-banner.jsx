...
function persistCookiePrefs(prefs) {
  ...
  window.dispatchEvent(
    new CustomEvent('tasi-cookie-consent-updated', { detail: prefs })
  );
}
...
export default function CookieConsentBanner() {
  ...
  const visible =
    mounted && pathname !== '/cookie-settings' && !dismissed && !hasStoredPrefs;
      ...
      <div className="mx-auto w-full max-w-5xl rounded-[10px] border border-slate-900/10 bg-slate-950 px-5 py-4 text-white shadow-[0_24px_60px_rgba(15,23,42,0.35)] ring-1 ring-black/5 dark:border-white/10 dark:bg-slate-900">
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between md:gap-5">
          <p className="text-sm leading-relaxed text-slate-100">
            We use essential cookies and optional cookies for analytics and
            personalization. Choose your preferences.
            <Link
              href="/cookie-settings"
              className="ml-1 font-semibold text-amber-300 underline underline-offset-2 transition hover:text-amber-200"
            >
              Cookie Settings
            </Link>
          </p>
