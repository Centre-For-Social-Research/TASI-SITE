    ...
    >
      ...
      <div
        className={cn(
          ...
          plan.highlighted &&
            'border-amber-200 bg-[linear-gradient(180deg,rgba(255,251,235,0.98),rgba(255,247,237,0.9))] dark:border-amber-700/40 dark:bg-[linear-gradient(180deg,rgba(120,53,15,0.35),rgba(17,24,39,0.92))]'
        )}
      >
        ...
        <div
          className={cn(
            'text-lg font-medium text-stone-900 dark:text-white',
            plan.highlighted && 'text-amber-950 dark:text-amber-100'
          )}
        >
          {plan.name}
        </div>
        ...
        {plan.price ? (
          <h3 className="mt-3 flex items-end gap-1">
            <span
              className={cn(
                'text-3xl font-bold text-stone-900 dark:text-white',
                plan.highlighted && 'text-amber-900 dark:text-amber-100'
              )}
            >
              {plan.price}
            </span>
      ...
      <div
        className={cn(
          ...
          plan.highlighted &&
            'bg-[linear-gradient(180deg,rgba(255,251,235,0.65),rgba(255,255,255,0.9))] dark:bg-[linear-gradient(180deg,rgba(120,53,15,0.18),rgba(15,23,42,0.55))]'
        )}
      >
        {plan.features.map((feature, index) => (
          ...
          >
            <CheckCircleIcon
              className={cn(
                'h-4 w-4 shrink-0 text-stone-900 dark:text-white',
                plan.highlighted && 'text-amber-700 dark:text-amber-300'
              )}
            />
            <TooltipProvider>
              <Tooltip delayDuration={0}>
                <TooltipTrigger asChild>
                  <p
                    className={cn(
                      feature.tooltip
                        ? 'cursor-help border-b border-dashed border-stone-400 dark:border-slate-500'
                        : ''
                    )}
                  >
                    {feature.text}
      ...
      {plan.btn ? (
        <div
          className={cn(
            ...
            plan.highlighted
              ? 'border-amber-200 bg-[linear-gradient(180deg,rgba(255,247,237,0.85),rgba(255,255,255,0.95))] dark:border-amber-700/40 dark:bg-[linear-gradient(180deg,rgba(120,53,15,0.16),rgba(15,23,42,0.92))]'
