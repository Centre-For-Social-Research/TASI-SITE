...
export default function RegistrationForm() {
  ...
  async function handlePhotoChange(event) {
    ...
    try {
      ...
      setPhotoNote(
        `Selected ${file.name} • ${dimensions.width} x ${dimensions.height} • ${Math.ceil(file.size / 1024)}KB`
      );
    } catch {
      setProfilePhoto(null);
      setPhotoNote(
  ...
  async function handleSubmit(event) {
    ...
    if (!profilePhoto) {
      setStatus({
        type: 'error',
        message: 'Please upload a valid profile photo before submitting.',
      });
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        setStatus({
          type: 'error',
          message: data.error || 'Unable to submit registration.',
        });
        return;
      }
      ...
      <form onSubmit={handleSubmit} className="mt-6 grid gap-4 md:grid-cols-2">
        ...
        <label className="flex flex-col gap-2">
          ...
          <select
            ...
            onChange={(event) =>
              updateField('attendee_category', event.target.value)
            }
          >
        ...
        <label className="md:col-span-2 flex flex-col gap-2">
          ...
          <textarea
            ...
            onChange={(event) =>
              updateField('attendance_reason', event.target.value)
            }
          />
        </label>
        <label className="md:col-span-2 flex flex-col gap-2">
