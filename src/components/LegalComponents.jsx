export function DocSection({ id, number, title, children }) {
  return (
    <section id={id} className="mb-12 scroll-mt-24">
      <p className="mb-1 text-xs font-semibold uppercase tracking-widest text-orange-700 dark:text-orange-300">
        {number}
      </p>
      ...
      <div className="space-y-3 text-[0.94rem] leading-relaxed text-slate-700 dark:text-slate-300">
        {children}
      </div>
    </section>
  );
...
export function Callout({ variant = 'teal', children }) {
  ...
  return (
    <div className={`${styles[variant]} my-4 rounded-r-lg px-4 py-3 text-sm`}>
      {children}
    </div>
  );
}
...
export function MetaBadge({ children, variant = 'default' }) {
  const styles = {
    default:
      'border-stone-300 bg-stone-100 text-stone-700 dark:border-zinc-700 dark:bg-zinc-900 dark:text-slate-300',
      ...
      <table className="w-full border-collapse text-sm">
        ...
        <tbody>
          {rows.map((row, rowIndex) => (
            <tr
              key={rowIndex}
              className="border-b border-stone-200 last:border-0 dark:border-zinc-800"
            >
              {row.map((cell, cellIndex) => (
                <td
                  key={cellIndex}
                  className="align-top px-3 py-2.5 text-slate-700 dark:text-slate-300"
                >
                  {cell}
                </td>
              ))}
