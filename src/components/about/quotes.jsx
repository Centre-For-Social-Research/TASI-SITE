    ...
    <section className="bg-white py-section-sm dark:bg-stone-950 md:py-section-lg">
      <div className="mx-auto w-full max-w-[1300px] px-4 md:px-8 lg:px-16">
        <div className="mb-12 max-w-3xl md:mb-16">
          <p className="mb-4 text-xs font-black uppercase tracking-[0.18em] text-rc-accent dark:text-orange-300 md:text-sm">Voices from TASI</p>
          <h2 className="text-4xl font-extrabold tracking-tight text-stone-900 dark:text-white md:text-5xl lg:text-[3.2rem]">What Leaders Said</h2>
          ...
          {quotes.map((quote) => (
            ...
            >
              <div className="mb-6 flex items-center gap-4">
                ...
                <div>
                  <p className="text-sm font-semibold text-stone-900 dark:text-white">{quote.author.split(",")[0]}</p>
                  <p className="text-xs leading-relaxed text-stone-500 dark:text-slate-400">{quote.author.split(",").slice(1).join(",").trim()}</p>
                </div>
              </div>
              <p className="text-body-lg leading-relaxed text-stone-800 dark:text-slate-200">{quote.text}</p>
              <cite className="mt-8 text-sm not-italic font-medium text-stone-500 dark:text-slate-400">{quote.author}</cite>
            </blockquote>
          ))}
        </div>
      </div>
