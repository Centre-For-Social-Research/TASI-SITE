...
import { ChevronLeft, ChevronRight, Linkedin, Mail, MoveHorizontal, Twitter } from "lucide-react";
          ...
          <div className="flex h-full w-full flex-col items-center justify-center space-y-4 text-center">
            ...
            >
              <Avatar className="h-32 w-32 ring-4 ring-stone-200 dark:ring-white/25">
                ...
                <AvatarFallback className="text-2xl text-stone-900">{initials}</AvatarFallback>
              </Avatar>
            </button>
            <div className="space-y-2">
              <h3 className="text-xl font-bold text-stone-900 dark:text-white">{name}</h3>
              <p className="text-sm text-stone-600 dark:text-white/75">{designation}</p>
            ...
            <p className="mt-2 text-xs text-stone-500 dark:text-white/65">Click photo to view bio</p>
          </div>
        </Card>
        ...
        >
          <div className="flex h-full flex-col">
            <div className="mb-4 flex items-center justify-between">
              <h3 className="text-xl font-bold text-stone-900 dark:text-white">Bio</h3>
              <button
                type="button"
            ...
            <div className="flex-1 overflow-y-auto">
              <p className="text-sm leading-relaxed text-stone-700 dark:text-white/85">{bio}</p>
            </div>
            <div className="mt-4 border-t border-stone-200 pt-4 dark:border-white/15">
              <div className="flex items-center space-x-3">
                <Avatar className="h-10 w-10">
                  <AvatarImage src={imageUrl} alt={name} className="object-cover" />
                  <AvatarFallback className="text-stone-900">{initials}</AvatarFallback>
                </Avatar>
                <div>
                  <p className="text-sm font-medium text-stone-900 dark:text-white">{name}</p>
                  <p className="text-xs text-stone-500 dark:text-white/70">{designation}</p>
                </div>
              </div>
            </div>
          </div>
...
export default function TeamGrid() {
  ...
  return (
    <section className="bg-white py-section-md dark:bg-[#121212] md:py-section-lg" id="team">
      <div className="mx-auto w-full max-w-[1400px] px-4 md:px-6">
        <div className="mb-8 flex flex-col gap-5 md:flex-row md:items-end md:justify-between">
          <div className="max-w-3xl">
            <p className="mb-2 text-body-xs font-semibold uppercase tracking-widest text-stone-500 dark:text-stone-400">Organizing Team</p>
            <h2 className="text-display-sm font-black tracking-tight text-stone-900 dark:text-white md:text-display-lg">The People Building TASI</h2>
