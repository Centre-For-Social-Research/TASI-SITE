...
export default function BuildMyAgenda({ sessions, isOpen, onClose }) {
  ...
  const handleGenerate = () => {
    ...
    if (selectedIds.size === 0) return setError("Please select at least one session.");
  ...
  const generatePDF = (userName, userEmail, selectedSessions) => {
    ...
    const tableData = selectedSessions.map(s => {
      ...
      const speakersStr = Array.isArray(s.speakers) ? s.speakers.join(", ") : (s.speakers || "");
    ...
    autoTable(doc, {
      ...
      styles: { cellPadding: 5, fontSize: 10, valign: 'middle', font: 'helvetica' },
      ...
      <div className="relative w-full max-w-3xl max-h-[90vh] bg-white dark:bg-stone-950 rounded-2xl shadow-2xl flex flex-col overflow-hidden border border-stone-200 dark:border-stone-800 text-stone-900 dark:text-stone-100">
        ...
        <div className="flex-1 overflow-y-auto p-6 flex flex-col gap-6">
          <p className="text-sm text-stone-600 dark:text-stone-400">
            Select the sessions you plan to attend and generate a personalized printable PDF agenda.
          </p>
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-xs font-semibold uppercase tracking-wider text-stone-500 mb-1.5">Email Address</label>
              <input 
                type="email"
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex flex-wrap items-baseline gap-x-2 gap-y-1 mb-1">
                              <span className="text-xs font-bold text-stone-900 dark:text-stone-300 shrink-0">{session.time}</span>
                              <span className="text-xs font-medium text-stone-500 px-1.5 py-0.5 rounded-md bg-stone-100 dark:bg-stone-800 shrink-0">{session.venue || session.track}</span>
                            </div>
                            <h4 className="text-sm font-semibold text-stone-800 dark:text-stone-200 leading-snug mb-1">{session.title}</h4>
                            {session.speakers && (
                              <p className="text-xs text-stone-500 dark:text-stone-400 line-clamp-1">
                                {Array.isArray(session.speakers) ? session.speakers.join(', ') : session.speakers}
                              </p>
                            )}
                          </div>
                        </div>
