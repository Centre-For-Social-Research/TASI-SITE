...
export default function ReceptionTicketing2026() {
  ...
  useEffect(() => {
    ...
    async function loadEvents() {
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error(data?.error || "Unable to load 2026 reception tickets.");
        }
        ...
        if (active) {
          setError(loadError instanceof Error ? loadError.message : "Unable to load 2026 reception tickets.");
        }
      } finally {
        if (active) {
          setLoading(false);
  ...
  const ticketTypes = useMemo(() => activeEvent?.ticketTypes || [], [activeEvent]);
  const selectedTier =
  ...
  async function handleCheckout(event) {
    ...
    try {
      ...
      if (data.freeOrder) {
        router.push(`/tickets/success?order=${encodeURIComponent(data.orderCode)}&free=1`);
        return;
      }
      ...
      const razorpay = new window.Razorpay({
        ...
        handler: async (paymentResult) => {
          ...
          router.push(`/tickets/success?order=${encodeURIComponent(data.orderCode)}`);
        },
      });
      ...
      <div className="mx-auto w-full max-w-[1300px] px-4 md:px-8 lg:px-16">
        <div className="mx-auto max-w-3xl text-center">
          ...
          <p className="mt-5 text-body-lg leading-relaxed text-stone-700 dark:text-slate-300">
            ...
            Standard supports your participation, Supporter extends that support,
            and Community keeps the experience accessible.
          </p>
          {demoMode ? (
            <p className="mt-4 rounded-[10px] border border-amber-200 bg-amber-50 px-4 py-3 text-sm font-medium text-amber-900 dark:border-amber-800 dark:bg-amber-950/40 dark:text-amber-200">
              Demo preview mode is active because Supabase ticketing credentials are not configured in this local environment yet.
            </p>
          ) : null}
        </div>
        ...
        {!loading && error ? (
          <div className="mt-12">
            <AdminAlert title="Ticketing unavailable" description={error} tone="danger" />
          </div>
        ) : null}
        ...
        {!loading && !error && !activeEvent ? (
          <div className="mt-12 rounded-[10px] border border-stone-200 bg-white p-8 text-center shadow-lg shadow-stone-200/40 dark:border-slate-800 dark:bg-slate-900">
            <p className="text-lg font-bold text-stone-900 dark:text-white">
              2026 reception tickets will appear here once the first event is published.
            </p>
          ...
          <>
            <div className="mt-10 text-center">
              ...
              <p className="mt-3 text-sm text-stone-600 dark:text-slate-300">
                {activeEvent.venue} · {new Date(activeEvent.startsAt).toLocaleString("en-IN", { dateStyle: "medium", timeStyle: "short" })}
              </p>
            </div>
            ...
            <div className="mt-10 grid gap-8 lg:grid-cols-3">
              {ticketTypes.map((ticketType) => {
                ...
                const available = ticketType.availability?.availableQuantity || 0;
                ...
                return (
                  <article key={ticketType.id} className="text-center">
                    ...
                    >
                      <div className={`relative h-[420px] px-7 py-6 text-left text-white ${getTicketSurface(ticketType.badgePattern)}`}>
                        ...
                        <div className="absolute inset-x-7 bottom-20">
                          ...
                          <p className="text-5xl font-black tracking-tight">{ticketType.name}</p>
              ...
              >
                <div className="grid gap-8 lg:grid-cols-[0.9fr_1.1fr]">
                  <div>
                    ...
                    <p className="mt-3 text-sm leading-relaxed text-stone-600 dark:text-slate-300">
                      {selectedTier.shortDescription || selectedTier.description}
                    </p>
                    ...
                    {selectedTier.ticketMode === "donation" ? (
                      <label className="mt-4 block text-sm font-semibold text-stone-700 dark:text-slate-200">
                        ...
                        <input
                          type="number"
                          min={Math.ceil((selectedTier.minDonationPaise || 0) / 100)}
                          step="1"
                          value={donationAmountInr}
                          onChange={(event) => setDonationAmountInr(event.target.value)}
                          className="mt-2 h-12 w-full rounded-[10px] border border-stone-200 bg-stone-50 px-4 text-sm text-stone-900 outline-none focus:border-amber-400 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100"
                        />
                      </label>
                    ) : null}
                    ...
                    <div className="mt-6 rounded-[10px] border border-stone-200 bg-stone-50 p-4 dark:border-slate-700 dark:bg-slate-800">
                      ...
                      <p className="mt-2 text-2xl font-black text-stone-900 dark:text-white">
                        {selectedTier.ticketMode === "free"
                          ...
                          : selectedTier.ticketMode === "donation"
                            ? donationAmountInr
                              ? formatCurrency(Number(donationAmountInr) * 100 * quantity)
                              : `From ${formatCurrency((selectedTier.minDonationPaise || 0) * quantity)}`
                            : formatCurrency((selectedTier.pricePaise || 0) * quantity)}
                      </p>
                    </div>
                  </div>
                    ...
                    <div>
                      ...
                      <div className="mt-4 grid gap-4 md:grid-cols-2">
                        <input
                          ...
                          onChange={(event) => setBuyer((current) => ({ ...current, fullName: event.target.value }))}
                    ...
                    <div>
                      ...
                      <div className="mt-4 space-y-4">
                        {attendees.slice(0, quantity).map((attendee, index) => (
                          <div key={index} className="rounded-[10px] border border-stone-200 bg-stone-50 p-4 dark:border-slate-700 dark:bg-slate-800">
                            <p className="text-xs font-black uppercase tracking-[0.18em] text-stone-500 dark:text-slate-400">
                              Ticket {index + 1}
                            </p>
                            <div className="mt-3 grid gap-3 md:grid-cols-3">
                              <input
                                ...
                                onChange={(event) =>
                                  setAttendees((current) => current.map((item, itemIndex) => itemIndex === index ? { ...item, fullName: event.target.value } : item))
                                }
                              ...
                              <input
                                ...
                                onChange={(event) =>
                                  setAttendees((current) => current.map((item, itemIndex) => itemIndex === index ? { ...item, email: event.target.value } : item))
                                }
                                className="h-11 rounded-[10px] border border-stone-200 bg-white px-4 text-sm text-stone-900 outline-none focus:border-amber-400 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100"
                              />
                              <input
                                ...
                                onChange={(event) =>
                                  setAttendees((current) => current.map((item, itemIndex) => itemIndex === index ? { ...item, phone: event.target.value } : item))
                                }
                                className="h-11 rounded-[10px] border border-stone-200 bg-white px-4 text-sm text-stone-900 outline-none focus:border-amber-400 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100"
                              />
                            </div>
