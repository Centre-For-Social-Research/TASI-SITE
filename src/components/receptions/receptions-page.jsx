...
import { MotionItem, MotionReveal, MotionStagger } from "@/components/home/motion-reveal";
        ...
        return (
          <button
            ...
            className={`rounded-full px-5 py-2 text-xs font-black uppercase tracking-[0.18em] transition ${
              active ? "bg-white text-[#140f26]" : "text-white/80 hover:text-white"
            }`}
          >
    ...
    <article className="rounded-[10px] border border-white/10 bg-white/[0.08] p-4 backdrop-blur-sm">
      <div className="flex items-center gap-4">
        ...
        <div className="min-h-[4.75rem] min-w-0">
          <h3 className="truncate text-base font-bold text-white">{person.name}</h3>
          <p className="mt-1 line-clamp-2 text-sm leading-relaxed text-white/70">{person.role}</p>
        </div>
      </div>
    </article>
  );
  ...
  return (
    <article className="flex h-full flex-col rounded-[10px] border border-stone-200 bg-white p-6 shadow-xl shadow-stone-200/40 transition-transform duration-300 hover:-translate-y-1 dark:border-slate-800 dark:bg-slate-900 dark:shadow-[0_18px_40px_rgba(0,0,0,0.25)]">
      <div className="flex items-center justify-between gap-4">
        <div>
          <p className="text-xs font-black uppercase tracking-[0.18em] text-rc-accent dark:text-rc-secondary">{reception.shortDate}</p>
          <h3 className="mt-3 text-2xl font-bold tracking-tight text-stone-900 dark:text-white">{reception.title}</h3>
        </div>
      ...
      <p className="mt-5 text-base font-semibold leading-relaxed text-stone-800 dark:text-slate-100">{reception.theme}</p>
      <p className="mt-4 text-sm leading-relaxed text-stone-600 dark:text-slate-300">{reception.venue}</p>
      <p className="mt-5 flex-1 text-sm leading-relaxed text-stone-600 dark:text-slate-400">{reception.summary}</p>
    ...
    <>
        ...
        <div className="mx-auto w-full max-w-[1300px] px-4 md:px-8 lg:px-16">
          <MotionReveal className="max-w-3xl">
            ...
            <h2 className="text-4xl font-extrabold tracking-tight text-stone-900 dark:text-white md:text-5xl lg:text-[3.1rem]">
              Three embassy evenings that extended the festival beyond the conference floor
            </h2>
            <p className="mt-5 text-body-lg text-stone-700 dark:text-slate-300">
              Each reception carried its own diplomatic tone, from pre-launch welcome to a women-led safety-by-design conversation and a forward-looking closing dialogue.
            </p>
          </MotionReveal>
            ...
            {receptionStats.map((item) => (
              <MotionItem key={item.label}>
                <article className="rounded-[10px] border border-stone-200 bg-white p-6 shadow-lg shadow-stone-200/40 dark:border-slate-800 dark:bg-slate-900 dark:shadow-[0_18px_40px_rgba(0,0,0,0.25)]">
                  <p className="text-4xl font-black tracking-tight text-rc-primary dark:text-rc-secondary">{item.value}</p>
                  <p className="mt-3 text-sm leading-relaxed text-stone-600 dark:text-slate-300">{item.label}</p>
                </article>
              </MotionItem>
            ))}
          </MotionStagger>
          ...
          <div className="space-y-10">
            {receptions.map((reception, index) => (
              <MotionReveal key={reception.slug} delay={index * 0.04}>
                <section id={reception.slug} className="overflow-hidden rounded-[10px] border border-stone-200 bg-[linear-gradient(180deg,#fffdf8_0%,#ffffff_100%)] shadow-xl shadow-stone-200/40 dark:border-slate-800 dark:bg-[linear-gradient(180deg,#111827_0%,#0b1220_100%)] dark:shadow-[0_18px_50px_rgba(0,0,0,0.28)]">
                  <div className="grid gap-0 lg:grid-cols-[1.18fr_0.82fr]">
                    <div className="p-6 md:p-8 lg:p-10">
                      ...
                      <p className="mt-7 text-body-lg leading-relaxed text-stone-700 dark:text-slate-300">{reception.summary}</p>
                      ...
                      <div className="mt-8 flex items-center gap-4 rounded-[10px] border border-stone-200 bg-white p-4 dark:border-slate-700 dark:bg-slate-900">
                        ...
                        <div>
                          <p className="text-xs font-black uppercase tracking-[0.18em] text-rc-accent dark:text-rc-secondary">Host Embassy</p>
                          <p className="mt-2 text-lg font-bold text-stone-900 dark:text-white">{reception.hostEmbassy}</p>
                          <p className="text-sm text-stone-500 dark:text-slate-400">Diplomatic partner for this reception evening</p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="grid gap-0 sm:grid-cols-2 lg:grid-cols-1">
                      {reception.gallery.map((image, imageIndex) => (
                        <div key={image} className={`relative min-h-[220px] ${imageIndex === 0 ? "lg:min-h-[220px]" : "lg:min-h-[220px]"}`}>
                          <Image
                            src={image}
                  ...
                  <div className="border-t border-stone-200 bg-[linear-gradient(135deg,#350265_0%,#5c0f4f_100%)] p-6 text-white dark:border-slate-800 md:p-8">
                    <p className="text-xs font-black uppercase tracking-[0.18em] text-rc-secondary">Speakers and key voices</p>
                    <h3 className="mt-3 text-3xl font-extrabold tracking-tight md:text-4xl">All speakers in this reception</h3>
      ...
      <section className="bg-[linear-gradient(120deg,#1b1035_0%,#3b0e5b_45%,#7a1e63_100%)] py-section-sm text-white md:py-section-lg">
        <div className="mx-auto grid w-full max-w-[1300px] gap-8 px-4 md:px-8 lg:grid-cols-[1.05fr_0.95fr] lg:px-16">
          <MotionReveal>
            <p className="text-xs font-black uppercase tracking-[0.18em] text-rc-secondary">Closing momentum</p>
            ...
            <p className="mt-5 text-body-lg leading-relaxed text-white/80">
              Across three evenings, TASI 2025 demonstrated that trust and safety leadership grows stronger when policymakers, embassies, civil society, and innovators gather in smaller rooms built for exchange, not just speeches.
            </p>
          </MotionReveal>
  ...
  return (
    <section className="bg-[linear-gradient(180deg,#fffdf8_0%,#f6efe6_100%)] py-section-sm dark:bg-[linear-gradient(180deg,#111827_0%,#0b1220_100%)] md:py-section-lg">
      <div className="mx-auto grid w-full max-w-[1300px] gap-8 px-4 md:px-8 lg:grid-cols-[1.1fr_0.9fr] lg:px-16">
        <MotionReveal>
          <p className="text-xs font-black uppercase tracking-[0.18em] text-rc-accent dark:text-rc-secondary">Future Access</p>
          <h2 className="mt-4 text-4xl font-extrabold tracking-tight text-stone-900 dark:text-white md:text-5xl lg:text-[3.1rem]">
            Future TASI receptions will remain intimate, curated, and access managed
          </h2>
          <p className="mt-5 text-body-lg leading-relaxed text-stone-700 dark:text-slate-300">
            Use this page as the premium front door for future diplomatic evenings, partner-hosted receptions, and limited-capacity networking moments around the festival.
          </p>
        ...
        <MotionStagger className="grid gap-5">
          {[
            ...
            <MotionItem key={item.title}>
              <article className="rounded-[10px] border border-stone-200 bg-white p-6 shadow-lg shadow-stone-200/40 dark:border-slate-800 dark:bg-slate-900 dark:shadow-[0_18px_40px_rgba(0,0,0,0.25)]">
                <p className="text-xs font-black uppercase tracking-[0.18em] text-rc-primary dark:text-rc-secondary">Future-ready</p>
                <h3 className="mt-3 text-2xl font-bold tracking-tight text-stone-900 dark:text-white">{item.title}</h3>
                <p className="mt-4 text-sm leading-relaxed text-stone-600 dark:text-slate-300">{item.text}</p>
              </article>
            </MotionItem>
          ))}
        </MotionStagger>
      ...
      <main className="bg-white text-stone-900 dark:bg-stone-950 dark:text-stone-100">
        <BrandedPageHero className="py-16 md:py-24">
          <div className="relative z-10 mx-auto w-full max-w-6xl px-4 text-center md:px-6">
            <p className="mb-4 text-xs font-black uppercase tracking-[0.18em] text-white/75">TASI Receptions</p>
            <h1 className="text-4xl font-black tracking-tight text-white md:text-6xl">
              ...
              <span className="mt-2 block text-[1.15rem] font-extrabold text-rc-secondary md:text-[1.9rem]">
                {is2025 ? "One diplomatic arc for digital safety." : "A future-facing hospitality programme for a landmark edition."}
              </span>
            </h1>
            ...
            <div className="mt-8 flex flex-wrap justify-center gap-3">
              ...
              <span className="rounded-full border border-white/20 bg-white/10 px-4 py-2 text-sm font-medium text-white shadow-sm backdrop-blur-sm">
                {is2025 ? "New Delhi diplomatic receptions" : "Reception programme in development"}
              </span>
              <span className="rounded-full border border-white/20 bg-white/10 px-4 py-2 text-sm font-medium text-white shadow-sm backdrop-blur-sm">
