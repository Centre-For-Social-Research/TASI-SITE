...
export async function upsertFestivalTicketUser(input) {
  ...
  const payload = {
    ...
    billing_country: String(input.billingCountry || "").trim().toUpperCase(),
    ...
    linkedin_url: sanitizeShortText(input.linkedinUrl || "", {
      fieldName: "LinkedIn URL",
      maxLength: 500,
      required: false,
    }) || null,
    profile_photo_path: sanitizeShortText(input.profilePhotoPath || "", {
...
export async function createFestivalTicket({ user, idempotencyKey }) {
  const supabase = getSupabase();
  const purchase = deriveFestivalTicketPurchaseDetails({ country: user.country });
...
export async function setFestivalTicketRazorpayOrderId(ticketId, razorpayOrderId) {
  const supabase = getSupabase();
...
export async function searchFestivalTickets(query) {
  ...
  const { data, error } = await supabase
    ...
    .select("*, user:festival_ticket_users(*)")
    .or(
      `ticket_number.ilike.%${sanitized}%,status.ilike.%${sanitized}%`,
    )
    .order("created_at", { ascending: false })
    .limit(20);
...
export async function completeFestivalCheckIn({ ticketId, operator, deskLabel, token }) {
  const current = await getFestivalTicketById(ticketId);
  ...
  await recordFestivalAdminAudit({
    ticketId,
    actionType: alreadyCheckedIn ? "festival_checkin_duplicate" : "festival_checkin_valid",
    operator,
    payload: {
      deskLabel,
      token,
