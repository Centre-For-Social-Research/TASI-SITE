...
function buildGuestInvitationEmail({
  ...
  const paragraphs = [
    ...
    'Please save the date. Practical festival information will be shared separately closer to the event.',
    'We look forward to welcoming you in New Delhi.',
  ];
  const normalizedSiteUrl = normalizeSiteUrl(siteUrl);
  const venueMapUrl =
